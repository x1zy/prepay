    ...
    return worker
      .start({
        onUnhandledRequest: (req) => {
          ...
          if (
            url.pathname.startsWith("/api/bicycle") ||
            url.pathname.startsWith("/prepay/api/bicycle")
          ) {
            return;
          }
      ...
      .catch((error) => {
        ...
        console.warn(
          "MSW will not intercept requests, but the app will continue to work",
        );
        console.warn(
          "Make sure mockServiceWorker.js exists in public/ directory",
        );
      });
  }
}
